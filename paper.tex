...
\begin{lemma}\label{lem:describing_q_i}\todo{Check this and state uniform continuity lemma as appropriate}
...
\begin{proof}
  ...
  Then for each $i$ a geodesic from $g_i$ to $\zeta_i$ passes through the subcomplex
  of $H$ comprising all simplices that meet $C$.  After passing to a
  subsequence these geodesic rays converge to a bi-infinite geodesic passing
  through the star in $H$ of $C$, both end points of which must be $\xi$, which
  is a contradiction.
\end{proof}

\begin{lemma}\label{lem:uniform_continuity}
  For any point $\zeta \in \boundary G - \set\xi$, there is a neighbourhood $V$
  of $\zeta$ and a number $i_0$ such that $V \subset U_i$ for all $i \geq i_0$,
...
\begin{proof}
  ...
  there we may take $i_0$ such that $V \subset U_i$ for $i \geq i_0$. Enlarge
  $i_0$ so that for any $i \geq i_0$, $\dist([\xi, g_i], x) \geq ?$.
  ...
  Furthermore, $\beta_j(n_i) \in H$. It follows that $\dist(\beta_j(n_i), [z_1,
  z_2]) > \delta$ and so by considering a triangle with vertices $g_i$, $z_1$
  and $z_2$, $\dist(\beta_1(n_i), \beta_2(n_i)) \leq 2\delta$. The result

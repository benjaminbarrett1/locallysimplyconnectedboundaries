...
\begin{proposition}\label{prop:basic_homotopies}
  ...
  \from \posreals\to\posreals$ such that $\mu(\eta)\to0$ as $\eta\to0$ with the
  following property. Let $n \geq m_0$ and let $\gamma_0$ and $\gamma_1$ be
  maps from $\boundary\Delta\to\boundary G$. Let $f_0$ and $f_1$ be simplicial
  approximations to $p^\infty_n\composed \gamma_0$ and
  ...
  continuity condition: for any $x \in \boundary\Delta^2$ and any $s$ and $t$
  in $[0,1]$,
  \begin{equation*}
...
\begin{proof}
  ...
  \gamma_1$ from $J\times\{0\} \disjointunion \{1\}$ to $\boundary G$. Using
  $\ddag'_?$ we can extend $\gamma_0\disjointunion\gamma_1$ along $x \times
  [0,1]$ for each vertex $x$ of $J$ in such a way that the extension along each
  edge $x\times [0,1]$, when composed with $p^\infty_n$ admits a simplicial
  approximation connecting $f_0(x)$ to $f_1(x)$ after at most $L$ subdivisions
  of the interval.

  \todo{Continue here!}
  ,
  satisfies the conditions of Proposition~\todo{extending maps}, and so admits
